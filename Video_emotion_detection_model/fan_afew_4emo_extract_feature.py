...
from basic_code import load_4emo, util, networks_4emo
DEVICE = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

'''
For extract 4 emotions for input video dir
1. add input dir as argument , 1 batch is 1 result of video 
2. modify data_generator_4emo as without label
3. load trained model(model.load(...))
4. modify the  function def val ()
   take pred_score as 4 emotion feature
   no need AverageMeter(evaluate whole eval dataset acc)
'''
...
def main():
    ...
    ''' Load data '''
    root_train = './data/face/train_afew_4emo'
    list_train = './data/txt/afew_train_4emo.txt'
    batchsize_train= 48
    root_eval = './data/face/val_afew_4emo'
    list_eval = './data/txt/afew_eval_4emo.txt'
    batchsize_eval= 64
    train_loader, val_loader = load_4emo.afew_faces_fan(root_train, list_train, batchsize_train, root_eval, list_eval, batchsize_eval)
    ''' Load model '''
    _structure = networks_4emo.resnet18_at(at_type=at_type)
    _parameterDir = './pretrain_model/Resnet18_FER+_pytorch.pth.tar'
    model = load_4emo.model_parameters(_structure, _parameterDir)
    ''' Loss & Optimizer '''
